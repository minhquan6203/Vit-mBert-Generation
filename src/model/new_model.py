...
class VQA_Model(nn.Module):
    ...
    def forward(self, questions: List[str], images: List[str], labels: List[str] = None):
        ...
        if labels is not None:
            embedd_answers,answers_mask = self.text_embbeding(labels)
            answers_mask=answers_mask.squeeze(1).squeeze(1)
            ...
            out = self.decoder(fused_output,fused_mask,embedd_answers,answers_mask)
            ...
            predicted_tokens = self.tokenizer.batch_decode(predicted_ids.squeeze().tolist(), skip_special_tokens=True)
