...
#     def train_forward(self, optimizer, encoder_features: torch.Tensor, encoder_attention_mask: torch.Tensor, answer_ids: torch.Tensor=None):
#         optimizer.zero_grad()
        
#         # Add padding to labels
#         max_length = encoder_features.size(1)
#         padded_labels = F.pad(answer_ids, (0, max_length - answer_ids.size(1)), value=0)
#         outputs = self.layer_norm(encoder_features)
#         outputs = self.gen(inputs_embeds=outputs, attention_mask=encoder_attention_mask, labels=padded_labels)
#         outputs.loss.backward()
#         optimizer.step()
        
#         return outputs.logits, outputs.loss

#     def eval_forward(self, optimizer, encoder_features: torch.Tensor, encoder_attention_mask: torch.Tensor, answer_ids: torch.Tensor=None):
#         optimizer.zero_grad()
